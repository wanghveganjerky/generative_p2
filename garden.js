...
export default class Garden {
  ...
  setShape(el) {
    ...
    document.addEventListener("keydown", (e) => {
      if (/^[a-zA-Z0-9]$/.test(e.key)) { // check if the pressed key is alphanumeric
        this.activeShape.value = e.key; // change the active shape value
        this.renderShape(); // re-render the shape
      }
    });
  }
  ...
  renderShape() {
    ...
    if (descenders.includes(this.activeShape.value.toLowerCase())) {
      this.activeShapeEl.style.paddingBottom = "-10px";  // Adjust this value as needed
    } else {
      this.activeShapeEl.style.paddingBottom = "0px";
    }
